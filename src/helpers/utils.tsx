...
export const getWeb3 = () => {
  ...
  const myWeb3 = new Web3(
    new Web3.providers.HttpProvider(process.env.NEXT_PUBLIC_INFURA!)
  );
  return myWeb3;
};
...
/*export const isWeb3Unlocked = async () => {
  ...
  if (provider === window.ethereum) {
    ...
    if (network === CHAIN[SELECTED.CHAIN][process.env.NEXT_PUBLIC_ENV].networkID) {
...
export const getContractInstance = async (abiJSONName) => {
  ...
  const network = CHAIN[process.env.NEXT_PUBLIC_ENV!].networkID;
...
export const getContractAddressFrom = async (abiJSONName: string) => {
  let artifactJSON;
  if (process.env.NEXT_PUBLIC_ENV === "Testnet") {
    artifactJSON = require(`../abi_testnet/${abiJSONName}.json`);
  } else if (
...
export const getContractInstanceFrom = async (
  abiJSONName: string,
